        ...
        <div class="lg:grid lg:grid-cols-4 lg:gap-8">
                    ...
                    <form action="{{ route('jobs') }}" method="GET" class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="search" id="search"
                                ...
                                placeholder="e.g. Merchandiser" value="{{ request('search') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="location" id="location"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm"
                                placeholder="e.g. Dhaka" value="{{ request('location') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <select id="job_type" name="job_type"
                                ...
                                <option value="{{ $type }}" @if(request('job_type')==$type) selected @endif>{{
                                    ucfirst($type) }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="education_level" name="education_level"
                                ...
                                <option value="{{ $level->id }}" @if(request('education_level')==$level->id) selected
                        ...
                        <div>
                            <label for="salary_min" class="block text-sm font-medium text-gray-700">Minimum
                                Salary</label>
                            <input type="number" name="salary_min" id="salary_min"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm"
                                placeholder="e.g. 30000" value="{{ request('salary_min') }}">
                        </div>

                        <div>
                            <label for="salary_max" class="block text-sm font-medium text-gray-700">Maximum
                                Salary</label>
                            <input type="number" name="salary_max" id="salary_max"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm"
                                placeholder="e.g. 60000" value="{{ request('salary_max') }}">
                        </div>

                        <div>
                            <button type="submit"
                                class="inline-flex items-center justify-center w-full px-6 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                                Apply Filters
                            </button>
                        </div>
                    </form>
            ...
            <main class="mt-10 lg:col-span-3 lg:mt-0">
                ...
                <div class="space-y-4">
                    @forelse($jobs as $job)
                   <div
                        class="flex items-start md:items-center flex-col md:flex-row justify-between p-4 transition-shadow duration-200 bg-white border border-gray-200 rounded-xl sm:p-6 hover:shadow-md">
