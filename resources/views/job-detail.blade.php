...
<div class="py-12">
    <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
        ...
        @endif
        <div class="p-6 bg-white border border-gray-200 rounded-lg md:p-8 shadow-lg overflow-visible">
            {{-- Header --}}
            <a href="{{ route('jobs') }}"
                class="flex items-center mb-4 text-sm font-medium text-gray-600 no-print hover:text-gray-900">
                ...
                <div class="items-start justify-between mb-6 md:flex">
                    ...
                    <div class="flex-shrink-0 mt-4 flex-col text-left md:text-right md:mt-0 md:ml-6">
                        ...
                        <div class="flex items-center mt-6 gap-x-2">
                            <button id="apply-now-btn" type="button"
                                class="no-print flex-grow text-center px-5 py-2.5 rounded-md text-sm font-medium text-white bg-red-600 hover:bg-red-700 transition-colors">
                                Apply Now
                            </button>
            ...
            <div class="mt-8 space-y-8">
                ...
                <div class="p-6 rounded-lg bg-sky-50">
                    ...
                    <div class="grid grid-cols-1 text-sm sm:grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-4">
                        <div><span class="font-semibold text-gray-600">Job Type:</span> <span class="text-gray-800">{{
                                ucfirst($job->job_type) }}</span></div>
                        <div><span class="font-semibold text-gray-600">Employment Status:</span> <span
                                class="text-gray-800">{{ ucfirst($job->employment_status) }}</span></div>
                        <div><span class="font-semibold text-gray-600">Location:</span> <span class="text-gray-800">{{
                                $job->location }}</span></div>
                        <div><span class="font-semibold text-gray-600">Experience:</span> <span class="text-gray-800">{{
                                $job->experience_min }} to {{ $job->experience_max }}
                                years</span></div>
                        <div><span class="font-semibold text-gray-600">Age:</span> <span class="text-gray-800">{{
                                $job->age_min }} to {{ $job->age_max }}</span></div>
                        <div><span class="font-semibold text-gray-600">Gender:</span> <span class="text-gray-800">{{
                                ucfirst(str_replace('_', ' ', $job->gender_preference)) }}</span>
                        </div>
                        <div><span class="font-semibold text-gray-600">Salary:</span> <span class="text-gray-800">{{
                                $job->salary_min }} - {{ $job->salary_max }}</span></div>
                        ...
                        <div><span class="font-semibold text-gray-600">Job Nature:</span> <span class="text-gray-800">{{
                ...
                <section id="company-info" class="text-gray-700 bg-gray-100 border border-gray-200 p-6">
                    ...
                    <div class="mt-4 space-y-3">
                        ...
                        <p><span class="font-semibold">Website:</span> <a href="{{ $job->company->website ?? '#' }}"
                                target="_blank" class="text-red-600 hover:underline">{{ $job->company->website ?? 'N/A'
                                }}</a></p>
...
<div id="apply-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden p-4 bg-black bg-opacity-60">
    <div class="relative w-full max-w-lg p-8 bg-white rounded-lg shadow-xl">
        ...
        <p class="mt-2 text-sm text-gray-600">Please fill out the form below, upload your CV or résumé, and click the
            submit button.</p>

        <form action="{{ route('applications.store', $job->slug) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input type="text" id="full_name" name="full_name" placeholder="Full Name*"
                    class="w-full px-4 py-3 text-sm bg-gray-100 border-transparent rounded-md focus:bg-white focus:ring-2 focus:ring-red-500"
                    required>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" id="full_name" name="full_name" value="{{ Auth::user()->name }}"
                    class="w-full px-4 py-3 text-sm bg-gray-100 border-transparent rounded-md" readonly>
            </div>
            <div>
                <label for="email" class="sr-only">Email</label>
            ...
            <div>
                <div class="flex justify-center px-6 pt-8 pb-8 mt-1 border-2 border-gray-300 border-dashed rounded-md">
                    <div class="space-y-1 text-center">
                        ...
                        <div class="flex text-sm text-gray-600">
                            <label for="cv_file"
                                class="relative font-medium text-red-600 bg-white rounded-md cursor-pointer hover:text-red-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-red-500">
                ...
                <div id="file-upload-name"
                    class="flex items-center justify-between hidden p-3 mt-2 border border-gray-200 rounded-md bg-gray-50">
                    <div class="flex items-center">
                        <svg class="w-6 h-6 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                            <path
                                d="M3.75 2.116A2.116 2.116 0 002.116 3.75v12.5c0 1.17.946 2.116 2.116 2.116h12.5a2.116 2.116 0 002.116-2.116V7.884a2.116 2.116 0 00-.62-1.496L13.62 1.504A2.116 2.116 0 0012.125 1H5.884A2.116 2.116 0 003.75 2.116zM8.5 7.25a.75.75 0 00-1.5 0v5.5a.75.75 0 001.5 0v-5.5zM10.75 5a.75.75 0 01.75.75v8.5a.75.75 0 01-1.5 0v-8.5a.75.75 0 01.75-.75zm2.25 2.5a.75.75 0 00-1.5 0v5.5a.75.75 0 001.5 0v-5.5z">
                            </path>
                        </svg>
            ...
            <div class="flex items-center mt-6">
                <input id="agree" name="agree" type="checkbox"
                    class="w-4 h-4 text-red-600 border-gray-300 rounded focus:ring-red-500" required>
            ...
            <div class="flex justify-end pt-6 space-x-4">
                <button type="button" id="cancel-btn"
                    class="px-6 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">Cancel</button>
                <button type="submit"
                    class="px-6 py-2.5 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md hover:bg-red-700">Submit</button>
            </div>
        </form>
    </div>
</div>
